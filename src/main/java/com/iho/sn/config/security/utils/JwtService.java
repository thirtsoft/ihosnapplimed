package com.iho.sn.config.security.utils;
...
public class JwtService {
    ...
    private String generateToken(Map<String, Object> extraClaims, UserDetails userDetails) {
        addRolesClaim(userDetails, extraClaims);
    ...
    private void addRolesClaim(UserDetails userDetails, Map<String, Object> claims) {
        ...
        val collect = authorities.stream().map(GrantedAuthority::getAuthority).collect(Collectors.joining(", "));
        for (GrantedAuthority granted : authorities) {
            roles.append(granted.getAuthority()).append(",");
        }
